...
public class Parser{
  ...
  private void parseSimpleProduct(String[] components, String line) throws BadEntryException {
    ...
    Product product = _store.getProduct(idProduct);    // add code here 
    Partner partner = _store.getPartner(idPartner);
    _store.addBatch(price, stock, partner, product);//Product product = get Product in _store with productId;
  ...
  private void parseAggregateProduct(String[] components, String line) throws BadEntryException {
    ...
    // add code here to do the following
    if (!_store.getAllProductsWarehouse().containsKey(idProduct)){
      //!_store does not have product with idProduct
